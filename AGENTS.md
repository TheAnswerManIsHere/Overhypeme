...
which subsystems are sensitive, where the generated API-validation schemas
live, which panel is the async-status reference implementation, and which
shared modules a reviewer should know. `agents-core.md` and the shared docs
it routes to dereference these answers, so they bind every agent, not only
Claude.
