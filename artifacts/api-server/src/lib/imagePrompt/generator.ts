...
export function buildImagePromptUserMessage(input: ImagePromptGenerationInput): string {
  ...
  return [
    ...
    "- visualGoal: ONE short clause naming the image PAYOFF (what the viewer should understand). Do NOT describe the full scene, camera, wardrobe, lighting, or background, and do NOT duplicate keyVisualElements.",
    "- visualApproach: ONE short clause on how to STAGE that payoff (tone + staging logic). Must be tonally CONSISTENT with compiledPrompt.prompt and must NOT restate visualGoal or the whole scene.",
    ...
    "- keyVisualElements: 3-12 entries; concrete visible elements only (no abstract joke explanation, no policy language).",
    "- compiledPrompt.prompt OWNERSHIP: write ONLY the concrete scene. The compiler injects identity, reference-image, token, and text-policy language itself, so do NOT author any of these in compiledPrompt.prompt — they are stripped before the engine sees them: (a) face/identity/likeness preservation (\"preserve the … face\", \"recognizable face\", \"same person\", \"do not replace … with a human\"); (b) mentions of the uploaded/reference/source image or i2i/t2i; (c) NAME/SUBJ-style identity template tokens or \"interpret these terms\" clauses; (d) readable-text/logo/watermark policy. Describe what SHOULD be in the frame; the compiler owns the rest.",
