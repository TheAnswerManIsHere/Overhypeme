...
import { createGlobalLimiter, globalLimiterLogLevel, type GlobalRateLimiterOverrides } from "./lib/rateLimit";
...
// Public asset endpoints that social crawlers (Twitter/X, Facebook, Discord,
// Slack, iMessage) fetch when unfurling links. These responses MUST NOT
// carry a Set-Cookie header — Twitter/X explicitly treats any response with
// Set-Cookie as user-specific content and refuses to use it as an OG image
// (you get the small "no preview" card instead of the summary_large_image
// card). The csrf_token cookie is only needed by the SPA for double-submit
// CSRF protection on mutations; crawlers never mutate, so issuing it on
// these paths is pure breakage.
//
// Also: when a Set-Cookie header is present, Cloudflare downgrades
// `Cache-Control: public` to `private`, which makes the image uncacheable
// at the edge — a second-order disaster for OG performance.
...
/**
 * Builds a fresh Express app instance. This is a factory rather than a
 * module-level singleton specifically so the global rate limiter can be
 * isolated per test: `createGlobalLimiter()` (called below, once per
 * `createApp()` call) constructs a fresh limiter + store every time, and
 * `limiterOverrides` lets a test inject a low ceiling without ever mutating
 * shared module state that other test files in the same shard could
 * observe. The module still exports a `default` singleton (below) built by
 * calling this once, for the production entrypoint and for existing code
 * that imports the app directly.
 */
export function createApp(options: { limiterOverrides?: GlobalRateLimiterOverrides } = {}): Express {
  ...
  const requestLogger = pinoHttp({
    logger,
    // See globalLimiterLogLevel's doc comment (lib/rateLimit.ts) for why this
    // is needed on top of the limiter's own throttled log line.
    customLogLevel: (_req, res) => globalLimiterLogLevel(res),
    serializers: {
      req(req) {
        try {
          return {
  ...
  // the parser/auth chain — and the global rate limiter is mounted right after
  // it. Exempting healthz from the limiter does not make its request path
  // cheap on its own: the parser/auth chain below still runs cookie parsing
  // and authMiddleware for any OTHER exempt-adjacent request, and any caller
  // could otherwise force a session lookup even on a path meant to be free.
  // Registering the real handler this early, ahead of that chain, is what
  // actually makes the exemption honest. This imports the canonical
  // `healthzHandler` (routes/health.ts) rather than redefining one, so there
  // is exactly one implementation.
  app.get("/api/healthz", healthzHandler);
  app.head("/api/healthz", healthzHandler);

  // Coarse, API-wide rate-limiting backstop — satisfies CodeQL's
  // js/missing-rate-limiting check, which only recognizes specific packages
  // (this one) rather than this repo's existing DB-backed limiters. See
  // lib/rateLimit.ts's createGlobalLimiter for the ceiling, exemptions, and
  // store. Mounted early enough that a rejected request never pays for body
  // parsing, CSRF, or authMiddleware's DB session lookup, but late enough
  // that its 429 carries correct CORS headers (cors() already ran above).
  // Direct-passed to app.use(), not wrapped — CodeQL's query is
  // pattern-sensitive on this exact shape.
  const globalLimiter = createGlobalLimiter(options.limiterOverrides);
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    // Skip csrf_token cookie issuance on public asset GETs — see
    // isPublicAssetRequest's comment above for why.
