import { Router, type IRouter } from "express";
import healthRouter from "./health";
import authRouter from "./auth";
import factsRouter from "./facts";
import hashtagsRouter from "./hashtags";
import usersRouter from "./users";
import adminRouter from "./admin";
import adminEnginesRouter from "./adminEngines";
import adminImagePromptRouter from "./adminImagePrompt";
import adminReferenceResearchRouter from "./adminReferenceResearch";
import adminTaxonomyHealthRouter from "./adminTaxonomyHealth";
import aiRouter from "./ai";
import memesRouter from "./memes";
import pulidJobsRouter from "./pulidJobs";
import ogRouter from "./og";
import renderRouter from "./render";
import storageRouter from "./storage";
import stripeRouter from "./stripe";
import jobsRouter from "./jobs";
import affiliateRouter from "./affiliate";
import importRouter from "./import";
import localAuthRouter from "./localAuth";
import reviewsRouter from "./reviews";
import evalRouter from "./eval";
import shareRouter from "./share";
import shareCopyRouter from "./shareCopy";
import shareIntentsRouter from "./shareIntents";
import heroExamplesRouter from "./heroExamples";
import videosRouter from "./videos";
import videoJobsRouter from "./videoJobs";
import routeStatsRouter from "./routeStats";

const router: IRouter = Router();

router.use(healthRouter);
router.use(authRouter);
router.use(localAuthRouter);
router.use(factsRouter);
router.use(hashtagsRouter);
router.use(usersRouter);
router.use(adminRouter);
router.use(adminEnginesRouter);
router.use(adminImagePromptRouter);
router.use(adminReferenceResearchRouter);
router.use(adminTaxonomyHealthRouter);
router.use(aiRouter);
router.use(memesRouter);
router.use(pulidJobsRouter);
router.use(ogRouter);
router.use(renderRouter);
router.use(storageRouter);
router.use(stripeRouter);
router.use(jobsRouter);
router.use(affiliateRouter);
router.use(reviewsRouter);
router.use(evalRouter);
router.use(shareRouter);
router.use(shareCopyRouter);
router.use(shareIntentsRouter);
router.use(heroExamplesRouter);
router.use(videosRouter);
router.use(videoJobsRouter);
router.use(routeStatsRouter);
router.use(importRouter);

export default router;
