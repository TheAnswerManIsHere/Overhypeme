...
export function AiBgPicker({
  ...
  const handleGenerate = async () => {
    ...
    // Phase 2: when we have a reference upload, route through the pre-generate
    // confirmation modal so the user can see what the analyzer found and pick
    // the subject-render mode (human i2i / non-human i2i / t2i). The legacy
    // /generate path still handles Generic (no-reference) generation below.
    if (aiSubMode === "reference" && selectedRefUpload) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Phase 2 pre-generate confirmation modal — opens when the user clicks
          Generate with a reference image. The modal handles the
          analyze/generate-v2/poll lifecycle internally; on completion we
          re-fetch the user's reference-generated images so the gallery picks
          up the new render. */}
