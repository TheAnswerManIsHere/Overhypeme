...
export function StockImagePicker({ factId, selectedId, onSelect, onZeroStock }: Props) {
  ...
  // live preview canvas shows a black background. Centralized in
  // `useAutoSelectDefault` so every picker uses the same guard.
  useAutoSelectDefault<StockImage>({
    enabled: !isLoading && !isError && images.length > 0,
