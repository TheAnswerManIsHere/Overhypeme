...
const globalLimiter = rateLimit({
  ...
  // via `debug_mode_active`. Clamped to [1, MAX_GLOBAL_RATE_LIMIT] (round-5
  // + round-6 findings, P2): getConfigIntRaw only falls back to the default
  // on a parse failure (NaN) — a stored value of 0/negative/excessive would
  // parse successfully and pass straight through, and the migration
  // deliberately never rewrites an existing `value` (§4), so a stale
  // out-of-range row must be made harmless at both ends at read time, not
  // assumed fixed upstream.
  limit: () =>
    Math.min(MAX_GLOBAL_RATE_LIMIT, Math.max(1, getConfigIntRaw("global_rate_limit_max", 600))),
...
- `artifacts/api-server/src/lib/adminConfig.ts` — **`getConfigIntRaw` already exists** (bypasses the debug-mode overlay) and is now used for `global_rate_limit_max` (round-5 finding). Add **`getConfigBooleanFresh`** (round-4 finding): an uncached, always-live single-row read used only by `handler`'s dry-run check, reading `row.value` directly, never `row.debugValue`. **Also add single-flight refresh to `loadAll()`** (round-3 finding).
...
9. **Load budget — concrete numbers:**
   ...
   - Workload D: an over-limit burst proving response-path latency is unaffected by the fleet-metrics flush.
