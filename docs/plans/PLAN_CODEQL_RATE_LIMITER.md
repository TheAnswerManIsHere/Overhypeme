...
**Observable sink for `globalRateLimitMetrics`** (round-3 finding): an in-memory object nobody can read the value of isn't real telemetry. New `GET /api/admin/rate-limit-metrics` (behind the existing admin-auth middleware, alongside the other `/api/admin/*` routes) returns the current snapshot as JSON — a plain reachable endpoint, not a new metrics-exporter integration, matching the scale of what this actually needs.
...
- `artifacts/api-server/src/routes/admin.ts` — new `GET /api/admin/rate-limit-metrics` route (existing admin-auth pattern), the observable sink for `globalRateLimitMetrics`.
