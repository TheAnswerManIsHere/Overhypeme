...
- **A shared-NAT discriminator, with a decision rule (round-5/round-7 finding, P2; round-8 correction of where the identity comes from, P1).** Codex has now three times pushed on this, and each round found the previous answer inadequate for a different reason. Rounds 4-5 offered better *aggregation* (day buckets, a bucket-count gauge), which cannot separate "one scraper" from "many users behind one NAT" at all — both are a single high-count bucket. Round 7 added a real discriminator but sourced the identity from the **raw `sid` cookie, read before authentication** — and round 8 correctly showed that is worse than no signal.
  ...
  **Fixed by collecting the identity only after `authMiddleware` has validated it.** The counting middleware no longer parses cookies at all. Instead, when a request is over-limit **and dry-run lets it through**, `handler` stamps the bucket's key hash onto `res.locals.grlOverLimitBucket`; a small middleware registered **after `authMiddleware`** then reads `req.user?.id` — a value that exists only because a session row was found in the database — and records that validated identity against the stamped bucket. A request with a forged or absent `sid` contributes **no** identity, so it cannot move any bucket out of the single/anonymous class.
  ...
  Per over-limit bucket it tracks a `Set` of `hashIp`-salted, truncated **validated user ids**, capped at 16 entries per bucket and at 1,000 tracked buckets per process per flush interval (overflow counted, not silently dropped). Nothing identity-derived is persisted — at flush time each bucket reduces to one of three classes and only the class counts are written, as ordinary hour-bucketed rows:
...
- **Fixed with real, operation-scoped cancellation: `runBounded()`.** Every DB call this middleware makes goes through one helper in `globalRateLimitStore.ts`, which:
  1. **Bounds connection acquisition.** `pool.connect()` is raced against a connect deadline. If the deadline wins, the abandoned promise gets `.then((c) => c.release(new Error("grl: abandoned connect"))).catch(() => {})` attached — a **destructive** release, so a late-arriving client is destroyed rather than silently handed to the next borrower mid-abandonment.
  2. **Bounds execution, server-side.** On the checked-out client: `BEGIN` → `SET LOCAL statement_timeout = <ms>` → the statement → `COMMIT` (`ROLLBACK` on error). Postgres cancels the statement itself at the deadline and `pg` rejects with `57014` (`query_canceled`).
  3. **Bounds *every* phase, including the ones `statement_timeout` can't reach, by destroying the connection (round-8 finding).** The outer deadline no longer merely stops awaiting: it calls **`client.release(err)` with a truthy error**, which `pg-pool` routes to `_remove` → `client.end()` → socket destroyed. The Postgres backend then terminates, which cancels any in-flight statement *and* rolls back the open transaction — for a stalled `BEGIN`, `COMMIT`, or `ROLLBACK` alike, none of which `statement_timeout` covers. This is what makes "no outstanding database operation" true for the whole critical section rather than only its middle.
  4. **Leaves the shared pool untouched.** `SET LOCAL` is transaction-scoped and reverts at `COMMIT`/`ROLLBACK`, and a destroyed client is never returned to the pool at all, so no borrower can inherit a mutated `statement_timeout` or a half-open transaction. No change to `lib/db/src/index.ts`.

  Applied to the Store's `increment`/`decrement`/`resetKey`, the query inside `getGlobalRateLimitRolloutConfig()`'s single-flighted refresh, the metrics flush, and each purge batch.
- **The two deadlines have a defined relationship, not just two numbers.** The inner `statement_timeout` is set strictly below the outer wall-clock deadline, so the ordinary case (a slow statement) is always resolved by Postgres-side cancellation with the transaction cleanly rolled back and the client reusable. The outer deadline is reached only when a phase `statement_timeout` cannot govern has stalled, and its sole action is destruction — so there is no window in which a transaction is left neither committed nor rolled back with the client returned to the pool.
- §4/§5's tests assert the property, not the timing, and cover the phases round 7 missed: a statement forced past its deadline (`pg_sleep`) rejects in budget and `pg_stat_activity` shows no lingering activity; **a stalled `BEGIN`, a stalled `COMMIT`, and a stalled error-path `ROLLBACK` each leave no client, no open transaction, and no query behind after the caller returns** — verified from a *separate* database session, not the one under test. Plus a case confirming an unrelated slow query on the shared pool (simulating a migration/backfill) is unaffected.
...
- `artifacts/api-server/src/lib/globalRateLimitStore.ts` — new, and the home of every mechanism in §3:
  ...
  - `runBounded()` (round-7 finding, P1; round-8 correction, P1) — checked-out client + bounded `pool.connect()` race + `BEGIN`/`SET LOCAL statement_timeout`/`COMMIT`, and **destructive `client.release(err)` on the outer deadline** so a stalled `BEGIN`/`COMMIT`/`ROLLBACK` — which `statement_timeout` cannot govern — still leaves no live client, transaction, or query. **Replaces round 6's `withTimeout()`** (which cancelled nothing) and corrects round 7's non-destructive release. Used by every DB call here: Store methods, the rollout-config refresh, the metrics flush, each purge batch.
  ...
  - The bounded identity-discriminator map (≤16 identities/bucket, ≤1,000 buckets/interval, overflow counted) and its three-class reduction at flush time (round-5/round-7 shared-NAT finding), fed **only** by DB-validated `req.user.id` values collected after `authMiddleware` — never by the raw `sid` cookie (round-8 finding, P1).
...
- `artifacts/api-server/src/__tests__/globalRateLimitStore.test.ts` — new: Store unit tests (increment/decrement/resetKey semantics, both persisted columns salted, window/expiry rollover, the `keyPrefix` option isolating `grl:` from `grl:rescue:`), a real concurrency test, and:
  ...
  - **Cancellation (round-7 P1, extended round-8 P1):** a statement past its deadline rejects in budget and `pg_stat_activity` shows nothing lingering; **plus a stalled `BEGIN`, a stalled `COMMIT`, and a stalled error-path `ROLLBACK`, each asserting no client, open transaction, or query survives the caller's return** — verified from a separate database session, since the phases `statement_timeout` can't govern are exactly what round 7 missed. An abandoned `pool.connect()` race destroys its late-arriving client rather than returning it to the pool.
  ...
  - **Discriminator (round-7, extended round-8 P1):** the three-class reduction, the 16-identity and 1,000-bucket caps, the overflow counter — **and the forgery case: arbitrary, duplicate, prefix-colliding, and malformed `sid` cookies never move a bucket out of the single/anonymous class, while eight genuinely authenticated sessions do classify it `overlimit_shared`.**
