...
## A stacked PR gets no CI at all, and the silence looks like "not started yet"
...
branch never matches, and **no workflow run is ever created for it**.

The failure is that this looks identical to CI merely being slow.
`get_check_runs` returns `total_count: 0` — the same response as a run that
hasn't reported yet — so a reasonable reading is "wait a bit longer." On PR #490
that state persisted for the PR's whole life until it was retargeted, and the
diagnosis only came from `list_workflow_runs` showing that **no run had ever
existed** for the branch, rather than from anything the PR page said.

**How to tell the two apart:** zero check runs plus zero workflow runs for the
branch means the trigger never matched; zero check runs with a workflow run
present means it is genuinely pending. Check the second thing, not just the
first.

**Consequences worth knowing before stacking:**

- **The merge bar cannot be met while stacked**, because item 1 is CI green and
  there is no CI. That is not a defect to work around — it is the bar working.
...
deliberate. But the interaction with stacking is invisible from the PR page,
...
an assumption about how many elements it is folding. When the set was
effectively a singleton, `min` and `max` and `[0]` all agree, so the choice
looks arbitrary and is never revisited. Widen the set later and they diverge
silently, with no type error and no failing test.

Concretely, on PR #490: `acceptedAt` used `Math.min` over the qualifying
review passes. Correct while `qualifying` held one element. Round 3 then
introduced a rule that could match several — and `min` now selected the
*earliest* accepted response where the check needed the *latest*, so a stale
response could satisfy a live request. The widening commit and the reduction
were in different functions, and nothing connected them.
...
commit — not just the code you were editing. The reductions are the part that
silently keeps compiling.

**Why this is not just "write more tests":** the singleton-era tests still pass,
because they only ever construct one element. The defect is invisible until a
test is written for the *widened* case, and the person who widened it is the
one least likely to think of it (see *Fixing the flagged site and leaving its
siblings* for the same asymmetry in a different shape).
