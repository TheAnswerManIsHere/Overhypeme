...
- **Review-round budget — a mechanical stopping rule for review loops**
  ...
  the stopping rule moved onto the action path. Every loop now declares a
  budget before round 1 (3 internal / 5 product / uncapped-with-🛑-at-5 for
  auth, payments, migrations) and the guard **refuses the `@codex review` post**
  past it — released once by a fresh-context Fable adjudication, and after that
  only by David. Rounds are **counted fresh from a validated GitHub snapshot**,
