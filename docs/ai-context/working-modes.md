...
| **Loop-ledger records** — `.agents/metrics/loops/<pr>.json`, filed per *The loop ledger* below | **Write it, one review pass, ship regardless of findings.** Codex's automatic first pass happens; its findings get one triage — fix anything that changes a stored number or the record's own internal consistency if the author agrees, otherwise merge as written. No re-request, ever, even if that first pass finds something. | Criticality ≈ 1 (David, 2026-08-11, after PR #406 — the record for PR #398 — ran three review rounds on a JSON file with no product surface: the causal numbers were correct every round, only the prose explaining them kept needing polish). This is a self-measurement about an already-closed, already-merged loop; looping review on it is the same ceremony-mismatch the *transient process docs* row above exists to prevent, just on a record that happens to be kept rather than deleted. Unlike that row there is no read-only contract to police — a ledger record can't mutate live state — so there is no glaring-issue carve-out either: one triage, then ship, full stop. If the author's own judgment says a finding reveals something genuinely missed (not just an alternate phrasing), fix it in that one triage or flag it to David directly — never loop Codex again to confirm. |
