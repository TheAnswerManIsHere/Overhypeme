...
> **Canonical, authoritative test-running guide for the Replit environment.**
> This supersedes the earlier "use the ambient `DATABASE_URL` directly /
> `node --import tsx/esm --test ...`" guidance that used to live in this file —
> that approach is now **wrong** and writes test data into the live `public`
> schema. The commands below match what the real full-suite runner
> (`artifacts/api-server/scripts/run-tests-sharded.sh`) actually does.
...
commands or writing a `docs/<FEATURE>_TEST_RUN.md`.
