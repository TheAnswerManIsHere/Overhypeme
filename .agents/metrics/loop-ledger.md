...
| 15 | [#292](https://github.com/TheAnswerManIsHere/Overhypeme/pull/292) | prose/contract | 14 | 770 | 72 | 7 | 19 | 4 | 3 | 8 | 0 | 4 | **73.3%** | 2.3 | — | none | ✗ *adjudication pending* | Merged (login-redirect XSS/open-redirect fix). `rounds` is mechanically undercounted by 1 (true 8): round 8, against the final commit `02bdcac`, came back clean and posted as a plain issue comment rather than a formal review — the same `countRounds` gap confirmed on row 11 (#286) and row 13 (#288), now observed a third time. `review hrs` (2.3, open→round 7) is undercounted the same way; the true last engagement was round 8, shortly before merge. Classification (author, from a full re-read of all 19 threads): round 1 (1, new ground — the P1 dot-segment normalization bypass). Round 2 (4: 3 new ground — a UAT factual error, the dual-implementation duplication critique, a UAT overstatement, all first-reviewed content from the initial PR — plus 1 wrong-fix — round 1's fix didn't test encoded dot-segment spellings). Round 3 (1, wrong-fix — round 2's fix added the parity script but didn't wire it into CI). Round 4 (4, wrong-fix — this PR's own loop-ledger-obligation edits, made in earlier commits of this same PR, left row 13 stale, a stale cohort-count sentence, an unjustified `not classified` deferral on rows 12–13, and #280 miscategorized in the wrong table). Round 5 (3: 2 propagation — round 4's fix broke a downstream population count and the row 6/7 superlative claims — plus 1 wrong-fix — round 4's new row 14 used an unverified `findings` figure). Round 6 (4, **invalid** — this review ran against the stale pre-fix commit `66c2780`, already superseded by `8ab18f6` at review time; the underlying claims were moot against the PR's actual state, not live defects. This session initially misjudged these as duplicate webhook deliveries of round 5 and said so with no action needed — genuinely wrong, caught only because David asked directly why comments were unanswered; see the corrected reply thread for the acknowledgment). Round 7 (1, wrong-fix — round 4's fix missed a second, duplicate "180 findings" reference alongside the one it corrected). Author classification only; independent blind adjudication not yet run — flagged as a follow-up, not done inline while unblocking PR #295's CI gate. |
| 16 | [#290](https://github.com/TheAnswerManIsHere/Overhypeme/pull/290) | prose/contract | 7 | 217 | 36 | 7 | 20 | 10 | 2 | 8 | 0 | 0 | **50.0%** | 1.0 | — | none | ✗ *adjudication pending* | Merged (`/document` harvest for #286's ledger backfill + CI guard). Backfilled by a different session than the one that ran this loop — found while unblocking PR #292/#295 on the loop-ledger coverage gate — from a full re-read of all 20 threads and their replies (which are unusually self-narrating: several round replies explicitly say what they fixed and why, since this loop's own subject is the ledger's methodology). Round 1 (4, all new ground — the initial PR's cohort-backfill overclaim, a soft-cap arithmetic error, a false same-log-shape premise, an unstated guard cutoff). Round 2 (4: 3 new ground — first review of the feature/code cohort explanation, the cherry-picked trend rows, and a mischaracterized `countRounds` bias, none touched by round 1's fix — plus 1 wrong-fix — round 1's log-shape fix left a residual "no way to tell" phrase in frontmatter/MEMORY.md). Round 3 (0 findings). Round 4 (3: 2 new ground — a missed worst-case-figure citation and the original, unjustified #285 size-exception deferral, both first-reviewed here — plus 1 wrong-fix — round 2's fix introduced a self-contradiction between its new "finding-bearing round" section and its own trend filter). Round 5 (3, all wrong-fix — round 4's fix mis-scoped the finding-bearing-round criterion three distinct ways: #270's known zero-finding round, an overly-absolute cohort claim, and the re-raised/propagation precondition). Round 6 (4: 3 wrong-fix, continuing round 5's rewrite — the wrong-fix-dominates claim, the every-finding-is-new-ground claim, an unverified-boundary hedge — plus 1 new ground — a structural "keep analysis in one place" critique of content unchanged since round 1). Round 7 (2, both propagation — round 6's own trimming fix silently reverted two things rounds 1 and 2 had already fixed). Round-attribution judgment calls here are less certain than a same-session classification would be (no access to the original session's intent, only the evidence trail); adjudication is the check on that, not yet run — flagged as a follow-up. |

### Rounds undercounted when a re-review is clean

**`rounds` and `review hrs` are understated for row 11 (#286), and the mechanism is now understood well enough to name.** After the round-1 fix (`6b04de2`), an explicit `@codex review` trigger drew a genuine second review from Codex against the fix commit — confirmed by its own "Reviewed commit: `6b04de28e6`" line — that found nothing further. That clean result posted as a **plain issue comment** ("Codex Review: Didn't find any major issues. Delightful!"), not as a formal `pull_request_review` submission. `countRounds` and `reviewInterval` only scan the `reviews` collection, so this real reviewer engagement is invisible to both: `rounds` reports 1 where the true reviewer-engagement count is 2, and `review hrs` reports 0.1 (PR-open → the round-1 review) where the true interval runs to the round-2 comment, roughly 2.8 hours. Row 15 (#292) shows the identical gap a third time (true rounds 8, not 7), confirming this is a recurring transport behavior — a clean re-review tends to post as a plain comment — worth promoting to a documented `loop-metrics.mjs` bias once one more instance is observed, per row 13's note.
...
row note). Naming all nine, not a subset, matters: #270 64.7%, #274 68.4%,
...
in every one), #289 (2 rounds, findings in every one), and #288 (5 rounds,
...
single six-point trend would repeat the error this section already exists to
correct.** David enabled Codex "Exhaustive code review" shortly after 6:56 PM
...
to 03:10) and #288's (2026-07-30T02:03 to 03:20) both fall entirely the
following day, well inside the new setting's window. An earlier draft of this
...
→ 72.1%, not falling.** The post-boundary population has grown from n=1 to
n=3 — #285 50.0%, #289 33.3%, #288 40.0% — its first real test rather than a
single unconfirmed point. All three sit below every pre-boundary loop's
share, and now as three independent loops from two different sessions rather
than one, which is more suggestive of exhaustive review's intended effect
(fewer self-inflicted findings once a round looks harder before stopping)
than the original single point was — but n=3 is still not enough to claim
the effect, and the two populations are not compared directly here, only
each against its own history.

**Two structural observations, now checked against six loops instead of
four:**

- **Round 1 is where new ground lives — holds in all six.** #282: 11/11;
  #274: 5/5; #285: 7/7; #289: 6/6; #288: 5/5 — every round-1 finding in every
...
- **Wrong fix dominates propagation in four of the six loops, not all six.**
  ...
  it. **#270 and #289 do not.** #270's reversal (18 propagation vs 4
  ...
  #270's, just in prose rather than an adapter. The dominance pattern holds
...
40.0% isn't monotonic either direction), not a finding. It is recorded here
