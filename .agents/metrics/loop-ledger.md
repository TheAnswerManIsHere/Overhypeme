...
| 11 | [#286](https://github.com/TheAnswerManIsHere/Overhypeme/pull/286) | prose/contract | 7 | 773 | 34 | 1 | 3 | 3 | 0 | 0 | 0 | 0 | **0%** | 0.1 | — | none | ✓ **0%** (3/3, full population) | Merged. The PR that built the ledger's own CI guard — so this row is the guard's first self-application, and it was **produced because the guard demanded it**, not because anyone remembered. All three findings landed on the auto-review of the original diff and all three are defects in the new guard script itself, each of the same shape: a check that could report success on data it had not actually validated (a typo'd count read as "not measured", a blank exemption reason accepted as a recorded decision, a duplicated row satisfying a presence-only lookup). That is a notable concentration — the instrument built to stop silent passes shipped three ways to pass silently, all caught on the first round. `review hrs` is a **measured** 0.1 (5m34s open→review), not an unmeasured blank. |
| 12 | [#289](https://github.com/TheAnswerManIsHere/Overhypeme/pull/289) | prose/contract | 8 | 146 | 45 | 2 | 9 | 6 | 2 | 1 | 0 | 0 | **33.3%** | 1.0 | — | none | ✓ **0%** (9/9, full population) | Merged. PR 0 of the documentation backfill (the manual's reading-order TOC, orientation, and putting `docs/manual/` behind the docs-accuracy gate). **The ledger's largest fully-concordant adjudication** — 0 of 9 disagreements. It is *not* the first: rows 5 (#276), 8 (#283), 9 (#284) and 11 (#286) are all fully concordant too, but at 1, 1, 1 and 3 findings respectively, where agreement is nearly free. Nine is the first count at which concordance carries real information. The comparison that is actually apt is within this row's own cohort — prose/contract: #270 14.7% (5/34), then 0% at #276, #283, #286 and here. (The plan-review rows — #274 5.3%, #282 18.6%, #285 8.3% — are a different cohort and not the right comparison set; an earlier draft of this note mixed #270 in with them and called this row a first, both of which the table above contradicts.) The likely reason for the agreement is legible rather than flattering: this loop's provenance was unusually easy to date, because round 1 reviewed the original diff and both propagation findings (A7, A8) attack a subsection that a *named round-1 fix* had authored, so "would this exist without that fix?" has an unambiguous answer. Contrast row 7's caveat, where the adjudicator reported it often could not date the offending text at all. Both propagations are factual errors inside content written mid-loop to satisfy an earlier finding — the same shape as #285's R5-5/R5-6, and the reason the backfill's method now requires a grounded claim table per chapter. The one wrong-fix (A9) is the recurring "fixed the section, left the parallel restatement" pattern, this time across two documents rather than within one. |
