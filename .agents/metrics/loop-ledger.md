...
| 12 | [#289](https://github.com/TheAnswerManIsHere/Overhypeme/pull/289) | prose/contract | 8 | 146 | 45 | 2 | 9 | 6 | 2 | 1 | 0 | 0 | **33.3%** | 1.0 | — | none | ✓ **0%** (9/9, full population) | Merged. PR 0 of the documentation backfill (the manual's reading-order TOC, orientation, and putting `docs/manual/` behind the docs-accuracy gate). **First fully-concordant adjudication in the ledger** — 0 of 9 disagreements, against 3/36, 5/34, and 16/86 on the plan-review rows. The likely reason is legible rather than flattering: this loop's provenance was unusually easy to date, because round 1 reviewed the original diff and both propagation findings (A7, A8) attack a subsection that a *named round-1 fix* had authored, so "would this exist without that fix?" has an unambiguous answer. Contrast row 7's caveat, where the adjudicator reported it often could not date the offending text at all. Both propagations are factual errors inside content written mid-loop to satisfy an earlier finding — the same shape as #285's R5-5/R5-6, and the reason the backfill's method now requires a grounded claim table per chapter. The one wrong-fix (A9) is the recurring "fixed the section, left the parallel restatement" pattern, this time across two documents rather than within one. |
