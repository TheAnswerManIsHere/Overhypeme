...
| 20 | [#294](https://github.com/TheAnswerManIsHere/Overhypeme/pull/294) | prose/contract | 13 | 613 | 142 | 13 | 56 | — | — | — | — | — | *not classified* | 31.4 | — | none | ✗ **not run** | Merged. `/document` harvest for PR #288 (async-queue hardening Phase 1), plus three more owed ledger rows (`check-ledger-coverage.mjs` pulled in #289/#290/#280 as rows 13–15 once this PR became "next"), plus a round-1 Codex doc-accuracy review fully addressed across the harvest content. **Mechanical columns only, causal classification deferred** — same posture as row 6 (#279) and row 14 (#280): this row's mechanical data (rounds, findings, artifact size, review interval) was derived from a subagent-assembled snapshot and confirmed self-consistent (`derive()`'s own findings-reconciliation check, which throws on any per-round mismatch, passed clean across all 56 findings), but the per-finding causal classification would have to be done from that same subagent's summarized dispositions rather than the raw thread text directly — a lower evidentiary bar than this ledger holds a measured causal figure to, so it is left unmeasured rather than produced at reduced confidence. Two patterns worth flagging for whoever picks this up: several findings recur as "fixed in file A, the identical claim still wrong in sibling file B" one or two rounds later (a `new_ground`/`propagation` boundary call each time — see row 13/#289's identical shape); and one finding (round 7) was rebutted, then reversed two rounds later (round 9) once the author concluded the original rebuttal itself was wrong ("You were right and I was wrong on the substance... I owe a correction, not another rebuttal") — a `wrong_fix`-shaped pattern applied to a rebutted-not-fixed finding, which the rubric's current categories don't cleanly cover. `pre-open preflight` unmeasured (`—`). |
