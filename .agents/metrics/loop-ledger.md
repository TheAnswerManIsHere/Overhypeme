...
over the gate, also `unmeasured`; see row 16's note), and #294 (now four
independent readings, no two of which agree on both the population size
and the propagation-vs-wrong-fix direction — `unmeasured`, see row 17's
note). Naming all ten, not a subset, matters: #270 64.7%, #274 68.4%, #276
0%, #282 72.1%, #283 0%, #285 50.0%, #286 0%, #288 35.0%, #289 33.3%, #297
0.0%. (A concurrent PR — #292 — independently derived #288 at 40.0%
instead of 35.0%; row 12's own note discloses both readings rather than
picking one silently. #292's #289 figure agrees exactly at 33.3%.)
...
- **Round 1 is where new ground lives.** In #282, round 1 was 11/11 new
  ...
  plan-review cohort, or any single loop's shape. (#294's round 1 is 9/9 new
  ground too, agreed by all four of its independent readings, but with the
  row's overall causal share `unmeasured`, it's noted in row 17 rather than
  added to this confirmed count.)
- **Wrong fix dominates propagation in four of the six loops, not all six.**
  ...
  **#294 is the closest this section has come to testing the dominance
  pattern on a genuinely large *pure-docs-harvest* loop the way #270 tested
  it for code, and its four independent readings don't even agree on the
  direction** — row 17's note discloses two propagation-dominant readings
  (22 vs 16, and 18 vs 17) and two wrong-fix-dominant readings (24 vs 9, and
  27 vs 6), the largest reversal-vs-dominance split in the ledger either
  way. Because the row's causal share is `unmeasured`, none of this is
  folded into a confirmed count the way #270/#289 are — it's the strongest
  evidence yet that a large enough loop can genuinely resist this pattern's
  premise, not just fail to replicate it. The dominance pattern holds
  reliably for plan-review loops (#274/#282) and for #288 — a 21-file,
  ...
  alongside code, not because it's a single-document edit), which is a
  genuinely different loop shape than a plan review. That #288 shows the
  same dominance despite the shape difference is a real replication, not a
  restatement of the plan-review case.

This is a hypothesis at n=3 pre-boundary (not falling) and n=2 confirmed
post-boundary (35.0% → 33.3%, declining slightly; #285 excluded as a
boundary straddle, #294 excluded as `unmeasured` — two separate issues
from, and not fixed by, the implementation-vs-docs cohort confound the
remaining pair still carries), not a finding. It is recorded here so the
...
Five of this file's twenty rows (#276, #283, #292, #297 — and #284 only
narrowly escaping) are bugfix-mode loops by intent. Four of them are
cohorted `prose/contract`, because `classifyCohort` checks for any `.md`
file **before** it checks the PR body's `**Fix tier:**` field, and a
bugfix PR routinely carries a doc — a UAT, a deferred-work entry, or (for
#297) the `working-modes.md` edit its own fix required. The rule is doing
