...
- **Suppress test noise**: Set `SENTRY_DROP_DEBUG_EVENTS=true` in production to prevent the `/api/admin/_debug/sentry` test button from creating real Sentry issues. When this env var is `true`, `beforeSend` in `instrument.ts` returns `null` for any event tagged `debug: sentry-test`, so the event is silently discarded before it reaches Sentry. The admin button still works (returns 500 as expected); only the Sentry issue creation is suppressed.
