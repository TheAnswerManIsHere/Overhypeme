...
- **Release name (CRITICAL)**: Must match between the SDK at runtime and the source-map upload plugin at build time, or Sentry can't symbolicate stack traces. To prevent drift, both sides derive the release from the same expression — `REPLIT_DEPLOYMENT_ID ?? REPLIT_GIT_COMMIT_SHA?.slice(0,7) ?? "dev"`. The frontend computes this once in `vite.config.ts` (variable `sentryRelease`), passes it to `sentryVitePlugin`, and injects it into the client bundle as `VITE_SENTRY_RELEASE` (read by `src/lib/sentry.ts`). The backend computes the same expression in `src/instrument.ts`; the esbuild plugin reads the same env vars. Do not introduce a separate release source — keep this single derivation.
...
- **Suppress test noise**: Set `SENTRY_DROP_DEBUG_EVENTS=true` (backend) and `VITE_DROP_DEBUG_EVENTS=true` (frontend) in production to prevent the admin Sentry test buttons from creating real Sentry issues. When `SENTRY_DROP_DEBUG_EVENTS=true`, `beforeSend` in `instrument.ts` returns `null` for any event tagged `debug: sentry-test`. When `VITE_DROP_DEBUG_EVENTS=true`, `beforeSend` in `src/lib/sentry.ts` does the same for the frontend. The admin buttons still work (backend returns 500; frontend still shows the ErrorBoundary); only the Sentry issue creation is suppressed.
- **End-to-end verification**: Once the DSN secrets are set, open `/admin` → "Sentry Error Reporting Test" section. Click **"Trigger backend Sentry error"** — this calls `POST /api/admin/_debug/sentry` (admin-only), which throws a synthetic error captured by `Sentry.captureException` (with tag `debug: sentry-test` and the admin user id) and also forwarded to the Sentry error handler via the Express error middleware. Click **"Trigger frontend Sentry error"** — this sets a React state flag that causes a throw *during the next render*, which the `Sentry.ErrorBoundary` wrapper in `App.tsx` catches and reports to the frontend Sentry project. Check both Sentry projects for a new issue within a few seconds. A 500 response from the backend endpoint is expected and correct (the route intentionally throws).
- **Source-map upload verified** (Task #122): With all four build-time secrets set, both production builds were run and confirmed:
  - Frontend (`PORT=26169 BASE_PATH=/ NODE_ENV=production pnpm --filter @workspace/overhype-me run build`): Vite plugin logged `[sentry-vite-plugin] Info: Successfully uploaded source maps to Sentry`. Two files uploaded (JS bundle + map). Zero `.map` files in `dist/public/assets/` after build.
  - Backend (`NODE_ENV=production pnpm --filter @workspace/api-server run build`): esbuild plugin logged `[sentry-esbuild-plugin] Info: Successfully uploaded source maps to Sentry`. Ten files uploaded (5 scripts + 5 maps). Zero `.map` files in `dist/` after build.
  - Remaining manual step (once deployed): trigger a test exception via the `/api/sentry-test` endpoint and confirm Sentry shows readable file/line numbers instead of minified column offsets.
